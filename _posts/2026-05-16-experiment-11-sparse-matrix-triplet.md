...
用 (行, 列, 值) 三元组紧凑表示稀疏矩阵，按行优先校验录入；位置编码 `i*n+j` 用 `long` 显式防溢出。
...
```powershell
gcc -finput-charset=UTF-8 -fexec-charset=UTF-8 homework/experiment_11.c -o homework/experiment_11.exe
.\homework\experiment_11.exe
```
