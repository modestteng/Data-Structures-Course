...
单文件版广义表实验：互相调用的解析器递归吃完嵌套括号，再用同样递归思路求复制、长度、深度。配套有自动化测试程序。
...
```powershell
gcc -finput-charset=UTF-8 -fexec-charset=UTF-8 homework/experiment_13.c -o homework/experiment_13.exe
.\homework\experiment_13.exe
```
